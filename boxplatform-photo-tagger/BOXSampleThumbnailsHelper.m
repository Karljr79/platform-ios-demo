...
- (void)storeThumbnailForItemWithID:(NSString*)itemID thumbnail:(UIImage*)thumbnail
{
    [self.thumbnailCache setObject:thumbnail forKey:[self keyForItemID:itemID]];
}

- (UIImage*)thumbnailForItemWithID:(NSString *)itemID
{
    return [self.thumbnailCache objectForKey:[self keyForItemID:itemID]];
}
...
- (NSString *)keyForItemID:(NSString *)itemID
{
    return [NSString stringWithFormat:@"%@_%@", itemID];
}
